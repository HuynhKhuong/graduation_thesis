...
void MainWindow::delay(int time_delay){
    QTime dieTime= QTime::currentTime().addSecs(time_delay);
    while (QTime::currentTime() < dieTime)
        QCoreApplication::processEvents(QEventLoop::AllEvents, 100);
}
void MainWindow::add_data(QString x, QString y, QString z){
    data<<QVector3D(x.toFloat(),y.toFloat(),z.toFloat());
    ...
    switch (data.action_id) {
        case DISPLAY_POSITION:{
            if(MovC_ACK == DISPLAY_ONLY){
                ...
                //add_data(ui->tb_x_cur_cor->text(),ui->tb_y_cur_cor->text(),ui->tb_z_cur_cor->text());
...
{
qDebug() << "button moveL clicked";
    flag_move = true;
    QByteArray command;
    command.append(START_CHAR);
    qDebug() << command.toHex() << command.size();
    command.append("00");
...
void MainWindow::on_bt_set_method_clicked()
{
    ...
    }else if(ui->rb_processing_gcode->isChecked() == true){
...
void MainWindow::on_bt_gcode_start_clicked()
{
    ...
    if(file_num > 1)
    {
        on_bt_gcode_configure_clicked();
        on_bt_load_offset_clicked();
        ui->tb_file_dir->setText("E:\\Qt\\Qt_Test\\SplitFile\\test_saved_"+QString::number(file_num)+".txt");
        on_bt_process_clicked();
        ui->rb_semi_auto->setChecked(true);
        on_bt_set_method_clicked();
        delay(1);
        on_bt_movL_clicked();
        delay(3);
        on_bt_movL_clicked();
        ...
        on_bt_set_method_clicked();
        log_console("E:\\Qt\\Qt_Test\\SplitFile\\test_saved_"+QString::number(file_num)+".txt");
        on_bt_connect_clicked();
        on_bt_connect_clicked();
        delay(1);
        on_bt_set_method_clicked();
    }else
        log_console(ui->tb_file_dir->text());
    file_num++;
    QByteArray command;
    command.append(START_CHAR);
    command.append("00");
...
{
qDebug() << "button configure clicked";
    flag_config = true;
    QByteArray command;
    command.append(START_CHAR);
    command.append("00");
...
{
qDebug() << "button load clicked";
    if(flag_move == true && flag_loadPoint == false && flag_start == true){
        ui->tb_z_offset->setText(QString::number((ui->tb_z_offset->text()).toDouble() + 0.1));
        ui->tb_z_cor->setText(ui->tb_z_offset->text());
        flag_loadPoint = true;
    }
    else if(flag_move == true && flag_loadPoint == true && flag_start == true){
        ui->tb_z_offset->setText(QString::number((ui->tb_z_offset->text()).toDouble() + 0.3));
        ui->tb_z_cor->setText(ui->tb_z_offset->text());
    }
    else
            ui->tb_z_cor->setText(ui->tb_z_offset->text());
...
{
    QString dir = QFileDialog::getOpenFileName(this, "Open A File", "E:\\test gcode\\Project1\\Project1");
    ui->txtFileDir->setText(dir);
}


void MainWindow::on_btnSave_clicked()
{
    QString dir = QFileDialog::getSaveFileName(this, "Save A File", "E:\\test gcode\\Project1\\Project1");
    QFile file(dir);
    QTextStream textStream(&file);

    if (!file.open(QIODevice::ReadWrite))
        QMessageBox::critical(this,"Open file error!","Check again!");
    else{
        QString str = ui->textEdit->toPlainText();
        textStream << str;
        file.close();
    }
    ui->textSplit->setText("File saved: " + dir);
}
...
{
    gcode->flag_split = true;
    QString dir = QFileDialog::getOpenFileName(this, "Open A File To Split", "E:\\test gcode\\Project1\\Project1");
    QFile inputFile(dir);
    double last_z = 0.1;
    QString s = "G0 X0.001 Y0.001 Z" + QString::number(last_z) + " F15.0\n", outtext="";
    QString out_line;
    if (inputFile.open(QIODevice::ReadOnly))
    {
       ...
       while (!in.atEnd())
       {
           ...
           out_line = gcode->Get_Temp_Test(line);
           qDebug() << gcode->r.x << gcode->r.y << gcode->r.z;
           if (last_z != gcode->r.z)
           {
               last_z = gcode->r.z;
               QString fileSplit = "E:/Qt/Qt_Test/SplitFile/test_saved_"+ QString::number(j)+ ".txt";
               qDebug() << fileSplit;
               QFile file(fileSplit);
               QTextStream textStream(&file);
               if (!file.open(QIODevice::ReadWrite))
                   QMessageBox::critical(this,"Open file error!","Check again!");
                else{
                   ...
                   file.close();
                   s.clear();
                   s += "G0 X0.001 Y0.001 Z" + QString::number(last_z) + " F15.0 \n";
               }
               j++;
               outtext += "File  " + fileSplit + "  " + QString::number(file.size()) +  "  bytes" + "\n";
           }
           s += out_line + "\n";
       }
...
{
    QString s="";
    QString fileName = ui->txtFileDir->toPlainText();
    ...
    if (inputFile.open(QIODevice::ReadOnly))
    {
       ...
       {
          QByteArray line =  in.readLine().toUtf8();
          if(gcode->Get_Temp_Test(line) != "empty")
            s = s + gcode->Get_Temp_Test(line)+ "\n";
       }
...
{
    QFile file("E:/Qt/Qt_Test/test.txt");
    file.remove();
}
...
{
    new_window = new temperature_controlpanel();
    new_window->show();
}
