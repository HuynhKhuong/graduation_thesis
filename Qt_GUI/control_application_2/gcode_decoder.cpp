...
Gcode_Decoder_DTC_TypeDef Gcode_Decoder::package_data(Gcode_Packet_Command_TypeDef execute_mode)
{
    ...
    std::vector<QList<GCode_Coordinate_TypeDef>> temp_execute = execute_data;

    while( temp_execute.size() > 0 )
    {
        int cnt = 0;
        while(execute_data.size()  > 2){
            execute_data.pop_back();
            cnt++;
        }
        ...
        std::sort(Z_cor.begin(), Z_cor.end(), [](double x, double y){ return (x<y);}); // arrange min -> max
        Min_Z = execute_data.at(0).at(0).Z;
        Max_Z = execute_data.at(execute_data.size()-1).at(0).Z;
qDebug() << Z_cor.size() << ": " << Z_cor ;
        //init first packet

        //seperate Z and packe
        ...
        if(execute_mode == GCODE_LINEAR){
            ...
            if(execute_data.size() == 1)
                ADD_VALUE(&temper_array, execute_data.at(0).size() , INT32_VALUE);
            else
                ADD_VALUE(&temper_array, execute_data.at(0).size() + execute_data.at(execute_data.size()-1).size()   +  - sub_point_gcode_smooth, INT32_VALUE);
        }
qDebug() << "compact_data.count() - 1 : "<< compact_data.count() - 1 << "  sub_point_gcode_smooth" << sub_point_gcode_smooth;
qDebug() << "execute_data.count()  : "<< execute_data.at(0).size() << "    " << execute_data.at(execute_data.size()-1).size() +  - sub_point_gcode_smooth << "  sub_point_gcode_smooth" << sub_point_gcode_smooth;
    ...
    //            }
                ...
                for(int j = start_index; j < execute_data.at(i).size(); j++){
                    ...
                    if(count == 19){
                        temper_array.append(RECEIVE_END);
                        PACKET_DEFINE_LENGTH(temper_array);
// qDebug() << "temper array "<< i << "," << j << ": "<< temper_array.toHex();
                        data_packet.append(temper_array);
...
//qDebug() << "temper array "<< i << "," << j << ": "<< temper_array.toHex();
            ...
            if(count != 0){ //uneven element in 1 packet
                ...
                PACKET_DEFINE_LENGTH(temper_array);
                data_packet.append(temper_array);
    //qDebug() << "temper array "<<  ": "<< temper_array.toHex();
                temper_array.clear();
            }
        }

        if(cnt == 1){
            temp_execute.erase(temp_execute.begin());
        }
        else if(cnt >= 2){
            temp_execute.erase(temp_execute.begin());
            temp_execute.erase(temp_execute.begin());
...
Gcode_Decoder_DTC_TypeDef Gcode_Decoder::LSPB_Process(double limit_angle)
{
    ...
    //calculate move time between point in each
    for(int i = 0; i < execute_data.size(); i++){
        double count_s = 0;
        for(int j = 1; j < execute_data.at(i).size(); j++){
            if(execute_data.at(i).at(j).Command == G02 || execute_data.at(i).at(j).Command == G03){
            ...
            qDebug() << "T: " << execute_data[i][j].T <<" --- s: " << count_s << "  Sa: " <<clutch_configure_data.at(i).Sa <<"  Sd: " <<clutch_configure_data.at(i).Sd  << " Total_s: " <<clutch_configure_data.at(i).total_s ;
        }
    }
...
QString Gcode_Decoder::Get_Temp_Test(QByteArray data)
{
    QStringList data_list;

    //QRegExp temperatureRegxp("[XYZE][-+]?(([0-9]\\d*\\.\\d*)|(0\\.\\d*[0-9]\\d*)|([0-9]\\d*))");
    QRegExp regXYZF("[XYZF][-+]?(([0-9]\\d*\\.\\d*)|(0\\.\\d*[0-9]\\d*)|([0-9]\\d*))");
    QRegExp regX("[X][-+]?(([0-9]\\d*\\.\\d*)|(0\\.\\d*[0-9]\\d*)|([0-9]\\d*))");
    QRegExp regY("[Y][-+]?(([0-9]\\d*\\.\\d*)|(0\\.\\d*[0-9]\\d*)|([0-9]\\d*))");
    QRegExp regZ("[Z][-+]?(([0-9]\\d*\\.\\d*)|(0\\.\\d*[0-9]\\d*)|([0-9]\\d*))");
    QRegExp regG("[G][-+]?(([0-9]\\d*\\.\\d*)|(0\\.\\d*[0-9]\\d*)|([0-9]\\d*))");
    QRegExp regF("[F][-+]?(([0-9]\\d*\\.\\d*)|(0\\.\\d*[0-9]\\d*)|([0-9]\\d*))");
    //  X

    int p = 0;
    if  (regXYZF.indexIn(QString(data),0)!= -1 & regG.indexIn(QString(data),0)!= -1)
    {
        p = 0;
        while ((p = regX.indexIn(QString(data), p)) != -1)
        {
            data_list.append(regX.cap(0));
            config.add_x =  regX.cap(0).remove(0,1);
            p+= regX.matchedLength();
        }
        //  Y
        p = 0;
        while ((p = regY.indexIn(QString(data), p)) != -1)
        {
            data_list.append(regY.cap(0));
            config.add_y =  regY.cap(0).remove(0,1);
            p+= regY.matchedLength();
        }
        //  Z
        p = 0;
        while ((p = regZ.indexIn(QString(data), p)) != -1)
        {
            data_list.append(regZ.cap(0));
            config.add_z =  regZ.cap(0).remove(0,1);
            p+= regZ.matchedLength();
        }
        p = 0;
        while ((p = regG.indexIn(QString(data), p)) != -1)
        {
            data_list.append(regG.cap(0));
            config.add_g =  regG.cap(0).remove(0,1);
            p+= regG.matchedLength();
        }
        p = 0;
        while ((p = regF.indexIn(QString(data), p)) != -1)
        {
            data_list.append(regF.cap(0));
            config.add_F =  regF.cap(0).remove(0,1);
            p+= regF.matchedLength();
        }
    }
    else return "empty";
    ...
    r.raw = QString(data);
    r.x =  config.add_x.toDouble();
    r.y =  config.add_y.toDouble();
    r.z =  config.add_z.toDouble();
    r.g =  config.add_g.toDouble();
    r.f =  config.add_F.toDouble();

    config.add_x = QString::number(r.x);
    config.add_y = QString::number(r.y);
    config.add_z = QString::number(r.z);
    config.add_g = QString::number(r.g);
    config.add_F = QString::number(r.f);
    //qDebug() << "Ma Gcode la: " << r.raw << " " << "So bien vi tri: "<<data_list.length();
   // qDebug() << data_list;
    QString out;
    if(flag_split == false)
       out = "G" + QString::number(r.g) + " X" + QString::number(r.x) +  " Y" + QString::number(r.y) +  " Z" + QString::number(r.z) + " F15.0";
    else
       out = "G" + QString::number(r.g) + " X" + QString::number(r.x) +  " Y" + QString::number(r.y) +  " Z0.0" + " F15.0";

    qDebug() <<  "G" << r.g << " X" << r.x <<  " Y" << r.y<<  " Z" << r.z << " F" << r.f;
    //return config.add_x + " " + config.add_y + " " + config.add_z ;
    return out;
}
